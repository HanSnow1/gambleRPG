...
| ID | Wired in |
|----|----------|
| aug_gilded_mirror | `PlayerAugmentState.OnAugmentAcquiredOrLeveled` → `PlayerRelicState.ApplyGildedMirror` |
| aug_anchor_point | `PlayerAugmentState.ApplyPreCombat` |
| aug_double_down | `AugmentCombatModifiers` + turn resolve |
| aug_crown_upgrade | `AugmentPickUI.ShowPick` → `AugmentSelector` tier bias |
| aug_second_wind / aug_last_chip | `PlayerAugmentState.TryPreventDeath` |
| aug_loaded_dice | `AugmentCombatModifiers.ModifyDamageOnSuccess` |
